...
def urg(d):
    return "נ”´" if d<=1 else "נ " if d<=3 else "נ¡" if d<=7 else "נ¢"
...
def check_memory(mem):
    ...
    for e in exams:
        d=days_until(e["date"])
        if d<=21: urgent.append(f"נ“– {e['course']} exam - {d}d")
    if urgent:
        lines.append("נ“ *Deadlines*")
...
def check_moodle():
    if not MOODLE_USER or not MOODLE_PASS:
        return "נ“ *Moodle* - credentials not set"
    try:
        s=requests.Session()
        ...
        if "loginerrormessage" in lr.text:
            return "נ“ *Moodle* - login failed, check credentials"
        lines=["נ“ *Moodle* - logged in"]
        ...
        if unread: lines.append(f"  נ’¬ {len(unread)} unread messages")
        return "\n".join(lines)
    except Exception as e:
        return f"נ“ *Moodle* - error: {str(e)[:60]}"
...
def check_gmail():
    if not GMAIL_ADDR or not GMAIL_PASS:
        return "נ“§ *Email* - credentials not set"
    try:
        ...
        if not ids:
            m.logout()
            return "נ“§ *Email* - no unread emails since yesterday"
        priority=[]
        other=0
        for eid in reversed(ids[-20:]):
            ...
            if any(p in sender.lower() for p in PRIORITY_SENDERS):
                ...
                priority.append(f"  נ“© {subject[:50]} - {src}")
            else: other+=1
        m.logout()
        lines=[f"נ“§ *Email* - {len(ids)} unread"]
        if priority:
        ...
        return f"נ“§ *Email* - login failed: {str(e)[:60]}"
    except Exception as e:
        return f"נ“§ *Email* - error: {str(e)[:60]}"
...
def check_aura():
    ...
    try:
        ...
        lines.append(f"  {'ג…' if r.status_code==200 else 'ג'} Homepage - {r.status_code} ({r.elapsed.total_seconds():.1f}s)")
    except Exception as e:
        lines.append(f"  ג Homepage unreachable: {str(e)[:40]}")
        return "\n".join(lines)
    for path,label in [("/login","Login")]:
        try:
            r2=ses.get(f"{AURA_URL}{path}",timeout=10)
            lines.append(f"  {'ג…' if r2.status_code==200 else 'ג ן¸'} {label} - {r2.status_code}")
        except: lines.append(f"  ג ן¸ {label} - unreachable")
    lines.append("  ג ן¸ Known risk: Firebase Auth in TikTok/Instagram browsers")
    if NETLIFY_TOKEN:
        try:
            ...
            if nr.status_code==200:
                ...
                if d:
                    st=d[0].get("state","?")
                    lines.append(f"  {'ג…' if st=='ready' else 'ג ן¸'} Netlify - {st} ({d[0].get('created_at','')[:10]})")
        except: pass
    else:
        lines.append("  ג„¹ן¸ Netlify token not set")
    try:
        ...
        if gr.status_code==200:
            ...
            if runs:
                ...
                lines.append(f"  {'ג…' if c=='success' else 'ג' if c=='failure' else 'נ”„'} GitHub Actions - {n} - {c or 'running'}")
    except: pass
    return "\n".join(lines)
